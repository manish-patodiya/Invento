$(function () {
    ...
    $('#frm-create-transaction').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: base_url('/transaction/dc/add_transaction'),
            beforeSend: function () {
            ...
            success: function (res) {
                if (res.status == 1) {
                    ...
                    setTimeout(() => {
                        window.location = base_url("/transaction/dc");
                    }, 2000);
                }
            },
            complete: function () {
...
$(function () {
    get_estimation_info(dc_invo_id);
})


function get_estimation_info(dc_invo_id) {
    if (dc_invo_id) {
        $.ajax({
            url: base_url('/transaction/cpo/get_DC_details_by_id'),
            type: 'post',
            data: {
            ...
            success: function (res) {
                if (res.status == 1) {
                    // estimation_details 
                    invo_info = res.data.cpo_details;
                    // estimation_product_details 
                    est_pro_info = res.data.cpo_product_details;
    ...
    $('#slc-est').on('change', function () {
        dc_invo_id = $(this).val();
        console.log(dc_invo_id);
        window.location = base_url("/transaction/dc/create_DC/" + dc_invo_id);
