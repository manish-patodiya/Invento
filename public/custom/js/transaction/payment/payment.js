$(function () {
    ...
    $('#frm-create-transaction').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: base_url('/transaction/payment/add_transaction'),
            beforeSend: function () {
...
//EST ...............................
$(function () {
    get_estimation_info(payment_invo_id);
})


function get_estimation_info(payment_invo_id) {
    if (payment_invo_id) {
        $.ajax({
            url: base_url('/transaction/payment/get_grn_details_by_id'),
            type: 'post',
            data: {
            ...
            success: function (res) {
                if (res.status == 1) {
                    ...
                    $('#choose-invo').val(invo_info.choose_invoice).trigger('change');
    ...
    $('#slc-est').on('change', function () {
        payment_invo_id = $(this).val();
        let branch_id = $('#branch-id').val();
        window.location = base_url("/transaction/payment/create_payment/" + branch_id + '/' + payment_invo_id);

    })
    $('#brid').on('change', function () {
        brid = $(this).val();
        window.location = base_url("/transaction/payment/create_payment/" + brid);
