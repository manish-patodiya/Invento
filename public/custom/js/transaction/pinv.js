$(function () {
    ...
    $('#frm-create-transaction').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: base_url('/transaction/pinv/add_transaction'),
            beforeSend: function () {
            ...
            success: function (res) {
                if (res.status == 1) {
                    ...
                    setTimeout(() => {
                        window.location = base_url("/transaction/pinv");
                    }, 10);
                }
            },
            complete: function () {
...
//EST ...............................
$(function () {
    get_estimation_info(pinv_id);
})


function get_estimation_info(pinv_id) {
    if (pinv_id) {
        $.ajax({
            url: base_url('/transaction/cpo/get_cpo_details_by_id'),
            type: 'post',
            data: {
                pinv_id: pinv_id,
            },
    ...
    $('#slc-est').on('change', function () {
        pinv_id = $(this).val();
        console.log(pinv_id);
        window.location = base_url("/transaction/pinv/create_Pinvo/" + pinv_id);
