$(function () {
    ...
    $('#frm-create-transaction').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: base_url('/transaction/returns/add_transaction'),
            beforeSend: function () {
            ...
            success: function (res) {
                if (res.status == 1) {
                    ...
                    setTimeout(() => {
                        window.location = base_url("/transaction/returns");
                    }, 2000);
                }
            },
            complete: function () {
...
$(function () {
    get_estimation_info(returns_id);
})


function get_estimation_info() {
    if (returns_id) {
        $.ajax({
            url: base_url('/transaction/returns/get_details_by_id'),
            type: 'post',
            data: {
            ...
            success: function (res) {
                if (res.status == 1) {
                    // Return_details 
                    invo_info = res.data.Return_details;
                    // Return_product_details 
                    est_pro_info = res.data.Return_product_details;
                    ...
                    $('#transaction-type-id').val(invo_info.trans_type_id);
                    $('#return-type').val(invo_info.choose_invoice).trigger('change');
                    ...
                    // lable changes for tranndaction type id  (grn for  vender)
                    let trans_type_id = invo_info.trans_type_id;
                    if (trans_type_id == 5) {
                        $('#change-transaction-type').html('Vender');
    ...
    $('#slc-est').on('change', function () {
        returns_id = $(this).val();
        brid = $('#branch-id').val();
        window.location = base_url("/transaction/returns/create_return/" + brid + '/' + returns_id);

    })

    $('#brid').on('change', function () {
        brid = $(this).val();
        window.location = base_url("/transaction/returns/create_return/" + brid);
    })
}