$(function () {
    ...
    $('#frm-create-transaction').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: base_url('/transaction/cpo/add_transaction'),
            beforeSend: function () {
            ...
            success: function (res) {
                if (res.status == 1) {
                    ...
                    setTimeout(() => {
                        window.location = base_url("/transaction/cpo");
                    }, 10);
                }
            },
            complete: function () {
...
//EST ...............................
$(function () {
    get_estimation_info(est_id);
})


function get_estimation_info(est_id) {
    if (est_id) {
        $.ajax({
            url: base_url('/transaction/est/get_estimation_details_by_id'),
            type: 'post',
            data: {
                est_id: est_id,
            },
            ...
            success: function (res) {
                if (res.status == 1) {
                    // estimation_details 
                    invo_info = res.data.estimation_details;
                    console.log(invo_info);
                    // estimation_product_details 
                    est_pro_info = res.data.estimation_product_details;
                        ...
                        $("#tbl-products > tbody").append(`
                            ...
                            <td class='text-end'>
                                ...
                                <div class='position-relative'>
                                    ...
                                    <input type="hidden" name='discount_amt[]'  id='discount-amt-${next_row_count}' class='discount-amt' value='${item.discount_amt}'>
                                </div>
                            </td>
                            <td class='text-end'>
    ...
    $('#slc-est').on('change', function () {
        est_id = $(this).val();
        window.location = base_url("/transaction/cpo/create_cpo/" + est_id);
