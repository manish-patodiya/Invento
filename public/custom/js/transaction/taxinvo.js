$(function () {
    ...
    $('#frm-create-transaction').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: base_url('/transaction/taxinvo/add_transaction'),
            beforeSend: function () {
            ...
            success: function (res) {
                if (res.status == 1) {
                    ...
                    setTimeout(() => {
                        window.location = base_url("/transaction/taxinvo");
                    }, 2000);
                }
            },
            complete: function () {
...
$(function () {
    get_estimation_info(tx_invo_id);
})


function get_estimation_info(tx_invo_id) {
    if (tx_invo_id) {
        $.ajax({
            url: base_url('/transaction/cpo/get_TXinvo_details_by_id'),
            type: 'post',
            data: {
    ...
    $('#slc-est').on('change', function () {
        tx_invo_id = $(this).val();
        console.log(tx_invo_id);
        window.location = base_url("/transaction/taxinvo/create_Txinvo/" + tx_invo_id);
