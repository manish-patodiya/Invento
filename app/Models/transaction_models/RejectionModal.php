...
class RejectionModal extends Model
{
    public $db;
    ...
    public function getAllInvoice($post_data)
    {
        ...
        $builder = $this->db->table('invoices');
        $builder->select('invoices.*, invoices.id as In_id,adr.*,adr.name as adr_name,mst.*,mst.status,mst.id as status_id')->join('address_book as adr', 'adr.id = invoices.branch_adrs_id', 'left')->join('master_status_trans as mst', 'mst.id=invoices.status', 'left');
        if ($bid) {
            $builder->where("invoices.branch_id= $bid");
        };
        if ($cust_name != "") {
        ...
        $builder->where("invoices.trans_type_id='5' AND invoices.status='3' OR invoices.trans_type_id='8'AND invoices.status='3'");
        return $builder->get()->getResult();
    }
    ...
    public function update_est_status($data, $id)
    {
        $data1 = [
