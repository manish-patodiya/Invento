...
class ReturnModal extends Model
{
    public $db;
    ...
    public function get_return_product_details($id)
    {
        $builder = $this->db->table('invoice_details');
        $builder->select('invoice_details.*,invoices.*,products.id,products.product_img, products.title as product_name, products.gst_rate,products.product_details,master_units.title as unit, hd.hsn_code_4_digits as hsn_code')
            ->join('products', 'products.id=invoice_details.product_id')
    ...
    public function get_all_grn_status_id()
    {
        $branch_id = $this->session->get('user_details')['branch_id'];
        $builder = $this->db->table('invoices');
        $builder->select('invoices.*')->where(['status' => 5, 'trans_type_id' => 5, 'branch_id' => $branch_id]);
        return $builder->get()->getResult();

    }

    public function update_db_status($data, $id)
    {
        $data1 = [
    ...
    public function return_type($id)
    {
        ...
        if ($id == 1) {
            $builder = $this->db->table('invoices');
            $builder->select('invoices.*')->where(['status' => 5, 'trans_type_id' => 5, 'return_type' => $id]);
        } elseif ($id == 2) {
            $builder = $this->db->table('invoices');
            $builder->select('invoices.*')->where(['status' => 5, 'trans_type_id' => 8, 'return_type' => $id]);
        }
        return $builder->get()->getResult();
