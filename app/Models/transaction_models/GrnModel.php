...
class GrnModel extends Model
{
    ...
    public function get_all_grn_id()
    {
        $branch_id = $this->session->get('user_details')['branch_id'];
        $builder = $this->db->table('invoices');
        $builder->select('invoices.*')->where(['status' => 5, 'trans_type_id' => 4, 'branch_id' => $branch_id]);
        return $builder->get()->getResult();

    }

    public function update_grn_status($data, $id)
    {
        $data1 = [
    ...
    public function update_grn_status_by_id($data, $id)
    {
        $data1 = [
            "updated_at" => date('Y-m-d'),
        ];
