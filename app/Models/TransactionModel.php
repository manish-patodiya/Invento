...
class TransactionModel extends Model
{
    ...
    public function getAllInvoice($post_data)
    {
        ...
        $builder = $this->db->table('invoices');
        $builder->select('invoices.*, invoices.id as In_id,adr.*,adr.name as adr_name,mst.*,mst.status')->join('address_book as adr', 'adr.id = invoices.branch_adrs_id', 'left')->join('master_status_trans as mst', 'mst.id=invoices.status', 'left');
        if ($tid != "") {
