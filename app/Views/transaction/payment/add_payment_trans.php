...
$branch_id = $session->get('user_details')['branch_id']
?>
...
let payment_invo_id = <?=$payment_invo_id?>;
</script>
        ...
        <div class="content-header">
            <div class="d-flex align-items-center row">
                <div class="me-auto col-md-4">
                    <h4 class="page-title"><i class="fa fa-plus"></i> Create Payment</h4>
                </div>
                <div class="col-md-8 ">
                    ...
                    <div class="box-header no-border">
                        ...
                        <div class="row mb-2">
                            ...
                            <div class='col-md-4'>
                                ...
                                <select class="form-control" id='choose-invo'>
                            ...
                            <div class="page-title col-md-4">
                                <label class="control-label" id='change-frist-lable'>Select payment type </label>
                                <select name="payment_invo_id" class='form-control' id="slc-est" disabled>
                        ...
                        <?php } else {?>
                        <input type="hidden" name='branch_id' id='branch-id' value='<?=$branch_id?>'>
                        <div class='d-flex justify-content-center mb-1'>
                            <div class="col-md-4 justify-content-center">
                                <label class="control-label">Select paymant type</label>
                                <select class="form-control" id='choose-invo'>
                                    <option value="">Select payment type </option>
                                    <option value="1">Outward</option>
                                    <option value="2">Inward</option>
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-6 text-end">
                                        ...
                                        <input name='create_date' type="date" class='d-none' id="chs-date"
                                            value='<?=date('Y-m-d')?>' />
                                        <label class="control-label" id='lbl-date'>:
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <?php } elseif ($trans_type == 5 || $trans_type == 6) {?>
                                        ...
                                        <div class="form-group">
                                            <span class=' row'>
                                                ...
                                                <span class="col-md-6 demo-checkbox d-none" id='checkbox-show'>
                                                    ...
                                                    <label for="basic_checkbox_1">Same As Billing Address
                                                    </label>
                                                </span>
                                            </span>
                                            <div class="controls">
...
?>
<script src="<?=base_url('public/custom/js/transaction/payment/comppayment.js')?>"></script>
<script src="<?=base_url('public/custom/js/transaction/payment/payment.js')?>"></script>
</script>
<script>
$(function() {
    ...
    $('#choose-invo').on('change', function() {
        let cho_id = $(this).val();
        let branch_id = $('#branch-id').val();
        $.ajax({
            url: base_url("/transaction/payment/choose_inovice"),
            method: "post",
            data: {
                id: cho_id,
                branch_id: branch_id,
            ...
            success: function(res) {
                if (res.status == 1) {
                    let select = res.payment_type;
                    let option = '';
                    payment_type_id = select[0]['choose_invoice'];
                    if (payment_type_id == 1) {
                        $('#slc-est').html('');
                        $('#slc-est').removeAttr('disabled')
                        $('#change-frist-lable').html('Select payment type with grn');
                        select.map(function(key) {
                            option +=
                                `<option value=${key.id} ${key.id==payment_invo_id? 'selected':false}>${key.invoice_prefix_id}</option>`;
                        })
                        $('#slc-est').append('<option value="">Select payment</option>' +
                            option);
                    } else if (payment_type_id == 2) {
                        $('#slc-est').html('');
                        $('#slc-est').removeAttr('disabled')
                        $('#change-frist-lable').html(
                            'Select payment type with tax invoice');
                        select.map(function(key) {
                            option +=
                                `<option value=${key.id} ${key.id==payment_invo_id? 'selected':false}>${key.invoice_prefix_id}</option>`;
                        })
